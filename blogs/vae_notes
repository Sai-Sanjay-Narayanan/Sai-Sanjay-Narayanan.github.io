...
One way of doing this is to first sample $z$ from a known distribution $r(z)$ (say, a standard Gaussian), and then sample $x \sim p_\theta(x|z)$, where $p_\theta$ is a learned network; and ideally

$$p(x) = p_\theta(x) := \mathbb{E}_{z \sim r(z)} \left[ p_\theta(x|z) \right]$$
...
(in practice, we use the empirical log-likelihood $\frac{1}{N} \sum_{i=1}^{N} \log p_\theta(x_i)$)

Now, computing $p_\theta(x) = \int p_\theta(x|z) r(z) dz$ is in general intractable, and

can only be estimated using Monte-Carlo integration:

$$p_\theta(x) \approx \frac{1}{M} \sum_{i=1}^{M} p_\theta(x|z_i), \quad z_i \sim r(z)$$

One might think of using the above Monte-Carlo estimate for each $x_i$ in the training set, and then maximize the (estimated) empirical log-likelihood. But, this approach is wrong because if $\hat{p}_\theta(x)$ is the Monte-Carlo estimate of $p_\theta(x)$, then indeed $\mathbb{E}_{z_i} [\hat{p}_\theta(x)] = p_\theta(x)$,

but $\mathbb{E}_{z_i} \left[ \log \hat{p}_\theta(x) \right] \neq \log p_\theta(x)$,
...
$$= \log \mathbb{E}_{z \sim r(z)} \left[ p_\theta(x|z) \right]$$

$$\geq \mathbb{E}_{z \sim r(z)} \left[ \log p_\theta(x|z) \right]$$
...
$$\geq \mathbb{E}_{x \sim p(x), z \sim r(z)} \left[ \log p_\theta(x|z) \right],$$

and so one might think of maximizing the lower bound

instead of $\mathcal{L}(\theta)$.

This looks nice, but it is actually not good. To see why, let us take a concrete example: Suppose $p_\theta(x|z)$ is modelled as a normal distribution $\mathcal{N}(\mu_\theta(z), \sigma^2 \mathcal{I})$. Then,
...
$$p_\theta(x) = \int p_\theta(x|z) r(z) dz$$

$$= \int p_\theta(x|z) \cdot \frac{r(z)}{q_\phi(z|x)} \cdot q_\phi(z|x) dz,$$

where $q_\phi(z|x)$ is another distribution parameterized by $\phi$. Thus,

$$p_\theta(x) = \mathbb{E}_{z \sim q_\phi(z|x)} \left[ p_\theta(x|z) \frac{r(z)}{q_\phi(z|x)} \right],$$

and by Jensen,

$$\log p_\theta(x)$$

$$\geq \mathbb{E}_{z \sim q_\phi(z|x)} \left[ \log p_\theta(x|z) + \log \frac{r(z)}{q_\phi(z|x)} \right],$$

and hence

$$\mathcal{L}(\theta) \geq \mathbb{E}_{x,z} \left[ \log p_\theta(x|z) \right]$$

$$- \mathbb{E}_{x} \left[ \mathbb{D}_{KL} \left( q_\phi(\cdot|x) \| r(\cdot) \right) \right]$$

where $(x, z) \sim p(x) q_\phi(z|x)$, i.e. they are no longer independently sampled. We can now maximize the RHS over $(\theta, \phi)$, and this is strictly better than the previous approach (indeed, if the space of all $q_\phi$ is rich enough, then it presumably
...
$$\mathbb{E}_{x,z} \left[ \log p_\theta(x|z) \right]$$

$$\propto \mathbb{E}_{x,z} \left[ -\|x - \mu_\theta(z)\|^2 \right]$$

Now, if we parameterize $q_\phi(z|x)$ as $\mathcal{N}(\lambda_\phi(x), \Delta^2 \mathcal{I})$, then
